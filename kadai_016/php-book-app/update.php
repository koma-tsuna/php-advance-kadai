...
// submitパラメータの値が存在するときの処理（「更新」ボタンを押したとき）
...
if (isset($_GET['id'])) {
    try {
        ...
        // 補足：fetch()メソッドは実行結果が取得できなかった場合にFALSEが返される
        if ($book === FALSE) {
            exit('idパラメータの値が不正です。');
        }

        // genresテーブルからgenre_codeカラムのデータを取得するSQL文を変数$sql_select_genre_codesに代入する
                ...
                <div>
                    ...
                    <select id="genre_code" name="genre_code" required>
                        ...
                        foreach ($genre_codes as $genre_code) {
                            // もし変数$genre_codeが商品の仕入先コードと一致していたら、selected属性をつけて初期値にする
